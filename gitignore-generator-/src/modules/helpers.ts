...
export async function getList(path: string | null, keepCurrent: boolean) {
    let data:any = await getData(`${API_URL}/list`);
    ...
    items.sort((a: { picked: any; }, b: { picked: any; }) => {
        if (a.picked) {
            return -1;
        } else if (b.picked) {
            return 1;
...
export function getUserRules(filePath:any) {
...
export function generateFile(path: string |null, output: string, override: boolean) {
    output = `# ${BANNER}\n${output}\n# ${USER_RULES}\n`;
